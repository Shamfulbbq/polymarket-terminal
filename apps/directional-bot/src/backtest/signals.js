...
export function momentum(candles, opts = {}) {
    ...
    if (Math.abs(pctChange) < threshold) {
        return { direction, confidence, features, skipped: true, skipReason: 'below_threshold' };
    }
...
export function takerBuyRatio(candles, opts = {}) {
    ...
    if (totalVol === 0) return { direction: null, confidence: 0, features: {} };

    const ratio = totalTakerBuy / totalVol;
    const direction = ratio >= 0.5 ? 'UP' : 'DOWN';
    const confidence = direction === 'UP' 
        ? Math.min((ratio - 0.5) / 0.2, 1)
        : Math.min((0.5 - ratio) / 0.2, 1);

    const features = {
        takerRatio: Math.round(ratio * 1000) / 1000,
        totalVol: Math.round(totalVol),
        totalTakerBuy: Math.round(totalTakerBuy)
    };

    if (ratio < buyThreshold && ratio > sellThreshold) {
        return { direction, confidence, features, skipped: true, skipReason: 'neutral_ratio' };
    }
...
export function vwapSignal(candles, opts = {}) {
    if (!candles || candles.length < 1) return { direction: null, confidence: 0, features: {} };
    ...
    if (cumVol === 0) return { direction: null, confidence: 0, features: {} };
    ...
    const features = {
        ...
        vwapDiff: Math.round(diffPct * 1000) / 1000,
        currentPrice
    };

    if (Math.abs(diffPct) < threshold) {
        return { direction, confidence, features, skipped: true, skipReason: 'near_vwap' };
    }
...
export function orderFlowComposite(candles, opts = {}) {
    ...
    const signals = [
        { name: 'vwap', sig: vwap,  weight: 0.30 },
        { name: 'mom',  sig: mom,   weight: 0.20 },
        { name: 'obi',  sig: obi,   weight: 0.20 },
        { name: 'cvd',  sig: cvd,   weight: 0.20 },
        { name: 'taker', sig: taker, weight: 0.10 },
    ];
    ...
    const features = {};
    for (const { name, sig, weight } of signals) {
        features[`${name}_dir`] = sig.direction;
        features[`${name}_conf`] = sig.confidence;
        if (sig.features) {
            Object.keys(sig.features).forEach(k => {
                features[`${name}_${k}`] = sig.features[k];
            });
        }
    ...
    const direction = upVotes === downVotes ? (weightedConf >= 0 ? 'UP' : 'DOWN') : (upVotes > downVotes ? 'UP' : 'DOWN');
    const rawConf = totalWeight > 0 ? Math.abs(weightedConf) / totalWeight : 0;
    const confidence = Math.min(rawConf, 1);

    features.upVotes = upVotes;
    features.downVotes = downVotes;
    features.weightedConf = weightedConf;

    const agreeing = Math.max(upVotes, downVotes);
    if (agreeing < (opts.minAgreement ?? 3)) {
        return { direction, confidence, features, skipped: true, skipReason: 'low_agreement' };
    }
...
export function preMomentumComposite(candles, opts = {}) {
    ...
    if (preCandles.length >= 2) {
        ...
        preMomPct = preMom.features?.pctChange || 0;
    }
    ...
    const votes = [
        vwapSig.direction ? { name: 'vwap', direction: vwapSig.direction, confidence: vwapSig.confidence, weight: 0.30, sig: vwapSig } : null,
        preMomDir ? { name: 'preMom', direction: preMomDir, confidence: preMomConf, weight: 0.25, pct: preMomPct } : null,
        obiSig.direction ? { name: 'obi', direction: obiSig.direction, confidence: obiSig.confidence, weight: 0.20, sig: obiSig } : null,
        cvdSig.direction ? { name: 'cvd', direction: cvdSig.direction, confidence: cvdSig.confidence, weight: 0.15, sig: cvdSig } : null,
        momSig.direction ? { name: 'mom', direction: momSig.direction, confidence: momSig.confidence, weight: 0.05, sig: momSig } : null,
        fundingDir ? { name: 'funding', direction: fundingDir, confidence: 0.5, weight: 0.05, rate: fundingRate } : null,
    ].filter(Boolean);

    const upVotes = votes.filter((v) => v.direction === 'UP').length;
    const downVotes = votes.filter((v) => v.direction === 'DOWN').length;
    
    const direction = upVotes === downVotes ? (upVotes > 0 ? 'UP' : null) : (upVotes > downVotes ? 'UP' : 'DOWN');
    const totalWeight = votes.reduce((s, v) => s + v.weight, 0);
    const weightedConf = votes.length > 0
        ? votes.reduce((s, v) => s + (v.direction === direction ? v.confidence * v.weight : -v.confidence * v.weight), 0)
        : 0;
    const confidence = totalWeight > 0 ? Math.min(Math.abs(weightedConf) / totalWeight, 1) : 0;
    ...
    votes.forEach(v => {
        features[`${v.name}_dir`] = v.direction;
        features[`${v.name}_conf`] = v.confidence;
    });
